...
#' @param scale Character. Time scale such as \code{"Daily"}, \code{"Weekly"}, or \code{"Monthly"}.
...
#' 
#' @export
dtn_prophetX_formula <- function(symbol, scale, date, field) {
  sprintf(
    '=IFERROR(RTD("prophetx.rtdserver","","AIHIST",%s,%s,"1",%d,"",%s,"XD"),0)',
    rb_excel_quote(symbol),
    rb_excel_quote(scale),
