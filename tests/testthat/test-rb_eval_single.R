...
# ## ---- Tests -----------------------------------------------------------------
# 
# test_that("rb_eval_single waits out 'Wait' and then returns values", {
#   skip_if_not(.Platform$OS.type == "windows", "Windows-specific test")
#   
#   sh <- make_fake_sheet()
#   
#   # updater simulates two ticks: first sets "Wait", then fills real values
#   tick <- 0L
#   updater <- function(sheet) {
#     tick <<- tick + 1L
#     if (tick == 1L) {
#       sheet[["Cells"]]$Item(1,1)[["Value2"]] <- "Wait"
#       sheet[["Cells"]]$Item(2,1)[["Value2"]] <- "Wait"
#     } else {
#       sheet[["Cells"]]$Item(1,1)[["Value2"]] <- 10
#       sheet[["Cells"]]$Item(2,1)[["Value2"]] <- "PX_LAST"
#     }
#   }
#   
#   fake_xl <- make_fake_excel_app(sh, updater)
#   
#   # stub rb_start_excel() so no real Excel is launched
#   stub(rb_eval_single, "rb_start_excel", function(visible = FALSE) fake_xl)
#   
#   out <- rb_eval_single(
#     formulas  = c("=SUM(1,2,3)", '=BDP("IBM US Equity","PX_LAST")'),
#     timeout_sec = 2,
#     visible   = FALSE,
...
#   expect_true(all(out$ready))
#   expect_identical(out$result[[1]], 10)
#   expect_identical(out$result[[2]], "PX_LAST")
# })
# 
