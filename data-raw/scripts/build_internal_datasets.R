rm(list=ls(all=TRUE));gc();library(data.table);library(dplyr)
...
source("data-raw/scripts/build_symbols_county_price.R")

source_files <- list.files("data-raw/internal_datasets/" ,full.names = T,pattern = ".rds$")
